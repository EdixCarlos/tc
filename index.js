...
const paddockManagers = [
  ...
  */
  ...
  function listPaddockManagersByName() {
    // CODE HERE
    // return paddockManagers.map((paddockManager) => paddockManager.name).sort();
  };
  ...
  function sortPaddockTypeByTotalArea() {
    // CODE HERE
    // return paddockType.map((paddockType) => paddockType.name).sort((a, b) => {
    //     return paddockType.filter((paddockType) => paddockType.name === b).reduce((total, paddockType) => total + paddockType.area, 0) - paddockType.filter((paddockType) => paddockType.name === a).reduce((total, paddockType) => total + paddockType.area, 0);
    //     });
  }
  ...
  function sortFarmManagerByAdminArea() {
    // CODE HERE
    // return paddockManagers.map((paddockManager) => paddockManager.name).sort((a, b) => {
    //     return paddockManagers.filter((paddockManager) => paddockManager.name === b).reduce((total, paddockManager) => total + paddockManager.area, 0) - paddockManagers.filter((paddockManager) => paddockManager.name === a).reduce((total, paddockManager) => total + paddockManager.area, 0);
    //     });
  }
  ...
  function farmManagerNames() {
    // CODE HERE
    return farms.map((farm) => farm.name).reduce((farmManagerNames, farm) => {
        farmManagerNames[farm] = paddockManagers.filter((paddockManager) => paddockManager.id === farm).map((paddockManager) => paddockManager.name).sort();
  ...
  function biggestAvocadoFarms() {
    // CODE HERE
    return farms.filter((farm) => farm.id === 1).map((farm) => farm.name).sort((a, b) => {
        return farms.filter((farm) => farm.name === b).reduce((total, farm) => total + 1, 0) - farms.filter((farm) => farm.name === a).reduce((total, farm) => total + 0, 0);
        });
  }
  ...
  function biggestCherriesManagers() {
    // CODE HERE
    return paddockManagers.filter((paddockManager) => paddockManager.id === 3).map((paddockManager) => paddockManager.name).sort();     
  }
  ...
  function farmManagerPaddocks() {
    // CODE HERE
    return paddockManagers.map((paddockManager) => paddockManager.name).reduce((farmManagerPaddocks, paddockManager) => {
        farmManagerPaddocks[paddockManager] = farms.filter((farm) => farm.id === paddockManager).map((farm) => farm.name).sort();
  ...
  function newManagerRanking() {
    // CODE HERE
    paddockManagers.push({  id: paddockManagers.length + 1,
